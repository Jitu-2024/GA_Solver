...
def run_gpu_ga_solver_background(
    ...
    try:
        ...
        # This block specifically catches permission-related issues, which are common on Windows.
        if "[WinError 5]" in str(e):
            error_message = (
                f"FATAL: A 'Permission Denied' error (WinError 5) occurred while trying to execute the solver at '{solver_path}'. "
                "This is a Windows file system permission issue. The web server process does not have the rights to run this executable. "
                "SOLUTION: Try running your terminal (e.g., PowerShell, CMD) as an Administrator and then starting the server again from there. "
                f"Detailed Error: {e}"
            )
            print(f"!!! {error_message} !!!")
            update_status("failed", error_message)
        else:
            # Handle other OS-level errors that aren't access denied
            update_status("failed", f"An unexpected OS error occurred: {str(e)}")
        return
    except Exception as e:
