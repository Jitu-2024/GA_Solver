...
__global__ void evaluateFitnessKernel(const size_t* flatArray, float* fitnessArray, size_t numGenomes,
                                       ...
                                       const float* jobTimes, size_t numJobs) {
...
void evaluateFitnessCPU(const std::vector<size_t>& flatArray, std::vector<float>& fitnessArray, size_t numGenomes,
                        ...
                        const std::vector<float>& jobTimes, size_t numJobs) {
    for (size_t genomeIdx = 0; genomeIdx < numGenomes; ++genomeIdx) {
        ...
        for (size_t i = 0; i < chromosomeLength; ++i) {
            ...
            // Debugging output for pickup phase
        ...
        // Debugging output for return to depot
        std::cout << "Genome " << genomeIdx << ", Return to Depot Travel Time: " << travelTimeToDepot
                  << ", Final Current Time: " << currentTime << std::endl;

        // Final fitness is the max of maxCompletionTime and currentTime
        fitnessArray[genomeIdx] = std::fmax(maxCompletionTime, currentTime);

        // Debugging output for final fitness
        std::cout << "Genome " << genomeIdx << ", Final Fitness: " << fitnessArray[genomeIdx] << std::endl;
    }
}

// Host function to flatten the population
void flattenPopulation(const std::vector<Genome>& population, std::vector<size_t>& flatArray, std::vector<float>& fitnessArray) {
    size_t numGenomes = population.size();
    size_t chromosomeLength = population[0].citySequence.size();

    flatArray.resize(numGenomes * chromosomeLength * 3); // City + Job + Pickup sequences
...
void evaluatePopulationFitness(std::vector<Genome>& population,
    ...
    for (size_t i = 0; i < numCities; ++i) {
        ...
        if (i > 0) { // Skip depot row for job times
            for (size_t k = 0; k < numJobs; ++k) {
                flatJobTimes[(i - 1) * numJobs + k] = jobTimes[i][k];
            }
        }
    }
    ...
    flattenPopulation(population, flatArray, fitnessArray);
    ...
    evaluateFitnessCPU(flatArray, fitnessArray, numGenomes, chromosomeLength, flatTravelTimes, numCities, flatJobTimes, numJobs);
    // Launch kernel
    int threadsPerBlock = 256;
    int blocksPerGrid = (numGenomes + threadsPerBlock - 1) / threadsPerBlock;
    evaluateFitnessKernel<<<blocksPerGrid, threadsPerBlock>>>(d_flatArray, d_fitnessArray, numGenomes, chromosomeLength, d_travelTimes, numCities, d_jobTimes, numJobs);
